...
def render_wtm_simulation_tab(graph: nx.Graph, config: SidebarConfig) -> None:
    st.subheader("WTM Cascade Results")

    if st.button("▶ Run simulation", key="wtm_run_sim"):
        n = graph.number_of_nodes()
        seed_size = max(1, int(config.seed_fraction * n))

        sim = WTMModel(graph, phi=config.phi)
        ...
        st.session_state[SessionKeys.WTM_SIM_RESULTS] = {
            "result": result,
            "metrics": metrics,
            "seed_size": seed_size,
            "n": n,
    ...
    if SessionKeys.WTM_SIM_RESULTS not in st.session_state:
        return

    sr = st.session_state[SessionKeys.WTM_SIM_RESULTS]
    result = sr["result"]
    metrics = sr["metrics"]
    n = sr["n"]
    ...
    m5.metric("Percolation Threshold (seed fraction)", f"{metrics.percolation_threshold:.4f}")
