...
def render_h2_simulation_tab(graph: nx.Graph, config: SidebarConfig) -> None:
    st.subheader("H2 Sequential Hybrid — Simulation Results")

    if st.button("▶ Run simulation", key="h2_run_sim"):
        ...
        st.session_state[SessionKeys.H2_SIM_RESULTS] = {
            "result": result,
            "metrics": metrics,
            "seed_size": seed_size,
            "n": n,
    ...
    if SessionKeys.H2_SIM_RESULTS not in st.session_state:
        return

    sr = st.session_state[SessionKeys.H2_SIM_RESULTS]
    result = sr["result"]
    metrics = sr["metrics"]
    n = sr["n"]
    ...
    c2.metric("Total Rounds", result.time_to_cascade)
    ...
    if result.switched:
        ...
        p1.metric("Switch triggered?", "✅ Yes")
        p2.metric("Infected at switch", f"{result.switch_fraction:.2%}")
        p3.metric("Phase 1 rounds (SIR)", result.rounds_phase1)
        st.metric("Phase 2 rounds (Bootstrap)", result.rounds_phase2)
    else:
        st.info(
            f"Switch threshold ({config.switch_fraction:.0%}) was never reached — "
            f"ran as pure SIR for {result.rounds_phase1} round(s)."
        )
    ...
    m6.metric("Switch Triggered (fraction of trials)", f"{metrics.switch_probability:.2%}")

    if metrics.avg_switch_fraction > 0.0:
        st.metric(
            "Avg Infected Fraction at Switch (switched trials only)",
            f"{metrics.avg_switch_fraction:.2%}",
        )
