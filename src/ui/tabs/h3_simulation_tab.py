...
def render_h3_simulation_tab(graph: nx.Graph, config: SidebarConfig) -> None:
    st.subheader("H3 Probabilistic Threshold Hybrid — Simulation Results")

    soft_threshold = round(1.0 / config.beta, 2) if config.beta > 0 else "∞"
    st.caption(
        f"Soft infection threshold: **m\* = 1/β = {soft_threshold}** infected neighbours "
        f"guarantee infection. Below this, infection is probabilistic."
    )

    if st.button("▶ Run simulation", key="h3_run_sim"):
        n = graph.number_of_nodes()
        seed_size = max(1, int(config.seed_fraction * n))

        sim = H3Model(graph, beta=config.beta, gamma=config.gamma)
        ...
        st.session_state[SessionKeys.H3_SIM_RESULTS] = {
            "result": result,
            "metrics": metrics,
            "seed_size": seed_size,
            "n": n,
    ...
    if SessionKeys.H3_SIM_RESULTS not in st.session_state:
        return

    sr = st.session_state[SessionKeys.H3_SIM_RESULTS]
    result = sr["result"]
    metrics = sr["metrics"]
    n = sr["n"]
