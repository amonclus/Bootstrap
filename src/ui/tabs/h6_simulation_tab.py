...
def render_h6_simulation_tab(graph: nx.Graph, config: SidebarConfig) -> None:
    st.subheader("H6 Probabilistic Threshold Results")

    if st.button("▶ Run simulation", key="h6_run_sim"):
        ...
        st.session_state[SessionKeys.H6_SIM_RESULTS] = {
            "result": result, "metrics": metrics, "seed_size": seed_size, "n": n,
        }

    if SessionKeys.H6_SIM_RESULTS not in st.session_state:
        return

    sr = st.session_state[SessionKeys.H6_SIM_RESULTS]
    result = sr["result"]
    metrics = sr["metrics"]
    n = sr["n"]
