...
def render_sis_simulation_tab(graph: nx.Graph, config: SidebarConfig) -> None:
    st.subheader("SIS Epidemic Results")

    if st.button("▶ Run simulation", key="sis_run_sim"):
        ...
        st.session_state[SessionKeys.SIS_SIM_RESULTS] = {
            "result": result,
            "metrics": metrics,
            "seed_size": seed_size,
            "n": n,
    ...
    if SessionKeys.SIS_SIM_RESULTS not in st.session_state:
        return

    sr = st.session_state[SessionKeys.SIS_SIM_RESULTS]
    result = sr["result"]
    metrics = sr["metrics"]
    n = sr["n"]
    ...
    c3.metric("Large Epidemic?", "✅" if result.is_large_cascade else "❌")

    st.markdown("#### Infected over time")
    if result.infected_series:
        fig = px.line(
            x=list(range(len(result.infected_series))),
            y=result.infected_series,
            labels={"x": "Round", "y": "Infected nodes"},
            title="SIS Epidemic Curve (single run)",
        )
    ...
    m1.metric("Peak Size (avg fraction)", f"{metrics.cascade_size:.4f}")
    m2.metric("Critical Seed Size", metrics.critical_seed_size)
    m3.metric("Epidemic Probability", f"{metrics.cascade_probability:.4f}")

    m4, m5 = st.columns(2)
    m4.metric("Time to Stabilise (avg rounds)", f"{metrics.time_to_cascade:.2f}")
    m5.metric("Epidemic Threshold (seed fraction)", f"{metrics.cascade_threshold:.4f}")
